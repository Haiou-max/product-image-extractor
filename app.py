...
def extract_from_image(client, image_bytes, media_type, model="claude-sonnet-4-20250514"):
...
if uploaded_files:
    ...
    if st.button("🚀 开始提取", type="primary", use_container_width=True):
        ...
        base_url = st.secrets.get("ANTHROPIC_BASE_URL", "https://api.evolink.ai")
        model_name = st.secrets.get("MODEL_NAME", "claude-sonnet-4-20250514")
        if not api_key:
        ...
        for idx, file in enumerate(uploaded_files):
            ...
            try:
                data = extract_from_image(client, img_bytes, media_type, model=model_name)
                results.append(data)
